 ...
 * <P>
 *     kafka client 发布record到kafka集群
 * The producer is <i>thread safe</i> and sharing a single producer instance across threads will generally be faster than
 * having multiple instances.
 * <p>
 *     producer是线程安全的，通常多线程共享一个producer实例会比多个producer实例更快
 * Here is a simple example of using the producer to send records with strings containing sequential numbers as the key/value
 * pairs.
 *
 *    这里是一个使用producer发送顺序的数字对字符串record的实例
 *
 * <pre>
 * {@code
 ...
 * <p>
 *
 *     producer 由一个buffer空间组成，这个buffer会保存那些还没有发送到server的record，同时在后台有一个io线程负责将record转到request里面并且把他们发送到集群中
 *
 *     在使用之后没有关闭producer会导致这些资源泄漏
 *
 * The {@link #send(ProducerRecord) send()} method is asynchronous. When called it adds the record to a buffer of pending record sends
 * and immediately returns. This allows the producer to batch together individual records for efficiency.
 * <p>
 *     send方法是同步的，会将record放入buffer中然后马上返回。这允许生产者将数据批处理以提高效率
 *
 ...
 * <p>
 *
 *     ack配置请求完成的标志，我们配置了all这个值将会阻塞直到完全提交了record。这是最慢但是最持久的设置
 *
 ...
 * <p>
 *     如果请求失败了，生产者会自动重试，如果配置了retries为0则不会重试。
 *
 ...
 * <p>
 *
 *     producer维护了一个buffer为每个partition保存了未发送的record，这些buffer可以通过batch.size配置。配置这个值更大可以同时批处理更多数据。
 *
 *     但是也会要求更高的内存。
 *
 ...
 * <p>
 *
 *     默认，一个buffer即使还有可用的空间也会马上发送消息。 如果你想减低请求数量，可以设置linger.ms为一个大于0的值。这会让producer在发送消息之前等待这么长时间以便
 *     更多的record被打包进一个batch中。这就好像tcp的Nagle's 算法一样。
 *
 *      在上面的代码片段中。设置了linger time 为1ms后，差不多100个record会被发送到单个request，然而，
 *      这个设置会在我们没有填满buffer的时候增加1ms的消耗让request等待更多的record
 *
 *      注意。级别安linger.ms设置为0的情况下，record也会及时到达并被批处理。所以在重负载下无论设置为多少，都会发生批处理
 *
 *      设置一个大于0的值，会导致在不是最大负载情况下，更高效的请求，更少的延迟。
 *
 *
 *
 ...
 * a TimeoutException.
 * <p>
 *     buffer.memory 控制procuder可用的buffer总内存。，如果生产速度大于消费速度，这会让buffer力不从心。
 *
 *     当buffer满了，后面的请求会被阻塞。阻塞的时间会在抛出TimeoutException后由max.block.mx控制
 *
 *
