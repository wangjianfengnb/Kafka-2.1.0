...
/**
  ...
  *
  * 这里的意思是说，这是Kafka 管理日志的入口，主要负责日志的创建，恢复，清理，所有读写操作都通过一个实例完成
  *
  * The log manager maintains logs in one or more directories. New logs are created in the data directory
  * with the fewest logs. No attempt is made to move partitions after the fact or balance based on
  * size or I/O rate.
  *
  * 日志文件会存在一个或多个文件夹中，新日志文件会在最少日志的目录下创建。意思就是说partitions创建之后就不会移动？
  *
  * 后台线程通过定时截断日志来保持日志
  *
...
class LogManager(logDirs: Seq[File],
  ...
  def startup() {
    ...
    if (scheduler != null) {
      ...
      // 把日志刷到磁盘中，这个定时任务的周期很长，为Long.MAX_VALUE.
  ...
  /**
    ...
    *
    * 把那些超过flush interval没刷到磁盘的日志刷入磁盘中。
    *
    */
