...
class Partition(val topicPartition: TopicPartition,
   ...
   *
   * 返回一个tuple第一个元素表示是否有足够的replica到达了requeireOffset第二个元素表示错误，Errors.NONE表示没有错误
   *
   ...
   * produce request.
   *
   * 注意这个方法只会在requireAcks=-1的情况下调用，在响应producer之前，我们会等待所有的replica完全到达leader的offset
   *
   */
  ...
  def getOutOfSyncReplicas(leaderReplica: Replica, maxLagMs: Long): Set[Replica] = {
     ...
     *
      * 如果说follower已经和leader同步了，就不会被标记为"不同步"的，否则有两种情况会在这里处理
      *  1. follower卡住了，如果replica经过了maxLagMs都没有更新，则表示follower需要移除
      *  2. "慢"的followers，如果relica超过maxLagMs都没有同步，也应该移除
      *
      * 两种方式都是通过检查lastCaughtUpTimeMs这个东西的，这个标志表示了最后follower心跳的时间，
      * 如果上述任何一种情况不符合，则需要移除
     **/
      // 首先排除掉leaderReplica
