...
class KafkaServer(val config: KafkaConfig, time: Time = Time.SYSTEM, threadNamePrefix: Option[String] = None,
  ...
  def startup() {
    try {
      ...
      if (canStartup) {
        ...
        // 获取brokerId和初始化的下线的log目录（下线就是说，发生IOException的目录）
        // 正常来说，这个是不存在的。
        ...
        // 这里屁股猜想一下，可能就是一些通过zk来动态修改config的一些feature
        ...
        // 一些调度东西，现在先不管吧。不知道什么意思
        quotaManagers = QuotaFactory.instantiate(config, metrics, time, threadNamePrefix.getOrElse(""))
        notifyClusterListeners(kafkaMetricsReporters ++ metrics.reporters.asScala)

        // 这个用来监控那些OfflineLogs目录的，如果发生IOException，会加入这里
        ...
        /*
         * 这里会起几个后台线程，定时清理log啥的，做一些checkpoint啥的
         */
        ...
        // 创建启动socket线程，同时延迟启动确保在验证的时候证书已经加载了
        socketServer = new SocketServer(config, metrics, time, credentialProvider)
        socketServer.startup(startupProcessors = false)

        /* start replica manager */
        // 这里也很关键，使用ReplicaManager来保持Partitions的replica同步。
        ...
        // 这里创建了一个Controller，会去判断说是否已经有Controller，以及一些选举的操作，
        // 这个Controller负责持有所有的topic，partition，broker的元数据
        ...
        // 一看名字就知道，初始化一些api之类的东西，负责接收client的请求的,比如创建topic啊，消费数据啊啥的
  ...
  /**
    ...
    *
    * 如果允许会生成brokerId或者按照下面条件从meta.properties中读取
    *
    * 1. 配置没有broker.id并且brokerid生成开启，通过zookeeper有序节点生成broker.id
    * 2. 保存在meta.properties中的brokerid和保存在log目录不匹配，抛出InconsistentBrokerIdException
    * 3. 配置有broker.id并且mata.properties也有id，但是不匹配抛出异常
    * 4. 配置有id，但是mate没有id，创建新的mate文件并保存id
    *
    * <ol>
