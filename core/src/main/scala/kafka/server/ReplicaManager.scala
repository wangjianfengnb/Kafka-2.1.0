...
class ReplicaManager(val config: KafkaConfig,
  ...
  // 这里的意思是说ReplicaAlterDirThread这个线程完成自己的任务后不会删除，所以需要周期性的shutdown这些线程
  ...
  def startup() {
    ...
    // 这里是前面声明的KafkaScheduler 其内部就是一个ScheduleThreadPoolExecutor

    // 默认 10秒 / 2 = 5秒执行一次，移除那些多余的replica
   ...
   */
  /**
    * 将消息填充到partition的leader replica中，然后等待复制到其他replica中，回调方法会在超时或者满足了acks数量之后被触发。
    *
    * 如果回调方法已经对某些对象加锁了，就会对这相同的object进行加锁避免死锁。
    */
  def appendRecords(timeout: Long,
    ...
    if (isValidRequiredAcks(requiredAcks)) {
      ...
      // 如果ack = -1 并且 partition不为空，而且最少一个写本地log的操作成功了
      // 就会等待replica同步复制
      if (delayedProduceRequestRequired(requiredAcks, entriesPerPartition, localProduceResults)) {
        ...
        // 尝试立即完成这个请求，如果完不成就放入 "炼狱" ？

        // 这是由于当延迟produce操作创建之后，后面的请求会到来让这个操作完成
  ...
  private def maybeShrinkIsr(): Unit = {
    trace("0 ISR list of partitions to see which replicas can be removed from the ISR")
